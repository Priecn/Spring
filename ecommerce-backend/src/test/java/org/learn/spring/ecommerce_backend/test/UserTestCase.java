...
public class UserTestCase {
	...
	/*
	 * @Test public void testAddAddress() { user = new User();
	 ...
	 * 
	 * address = new Address(); address.setAddressLineOne("E city");
	 * address.setAddressLineTwo("Near Infosys"); address.setCity("bagalore");
	 * address.setState("karnataka"); address.setCountry("India");
	 * address.setPostalCode("560100"); address.setShipping(true);
	 * address.setUser(user); assertEquals("Failed to add shipping address!", true,
	 * userDAO.addAddress(address)); }
	 */

	/*
	 * @Test public void testGetAddresses() { user =
	 * userDAO.getByEmail("hr@gmail.com");
	 * 
	 * assertEquals("Failed to fetch list of addresses!", 2,
	 * userDAO.listOfShippingAddresses(user).size());
	 * assertEquals("Failed to fetch billing addresses!", "Mumbai",
